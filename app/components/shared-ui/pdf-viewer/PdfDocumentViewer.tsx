...
export default function PdfDocumentViewer({
    ...
    return (
        <div className={cn("flex flex-col h-full transition-all duration-300",
            ...
            {/* Controls Bar */}
            <div className={cn("flex items-center justify-between px-4 py-3 z-10 shadow-sm transition-colors shrink-0",
                isFullscreen ? "bg-white/10 text-white rounded-xl mb-4 backdrop-blur-md border border-white/10" : "bg-white border-b border-gray-200"
            )}>
                <div className="flex items-center gap-2">
                    <span className={cn("text-sm font-bold px-3 py-1 rounded-full", isFullscreen ? "bg-white/20 text-white" : "bg-gray-100 text-lhbs-green")}>
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className={cn("p-2 rounded-full transition-colors", isFullscreen ? "hover:bg-white/20 text-white" : "hover:bg-gray-100 text-gray-600")}
                        title="Thu nhỏ"
                    >
                        <ZoomOut className="w-5 h-5" />
                    </button>
                    <span className={cn("text-xs font-medium min-w-[3rem] text-center", isFullscreen ? "text-white" : "")}>{Math.round(scale * 100)}%</span>
                    <button
            ...
            {/* Slider Area */}
            <div className={cn("relative flex-1 bg-gray-50/50 overflow-hidden group w-full flex flex-col",
                `pdf-container-${mode}`,
                    ...
                    <div className="overflow-hidden h-full " ref={emblaRef}>
                        <div className="flex touch-pan-y touch-pinch-zoom h-full items-start">
                            {Array.from(new Array(numPages), (el, index) => (
                                <div className={cn("flex-[0_0_100%] min-w-0 relative pl-4 flex justify-center h-full overflow-y-hidden",
                                    ...
                                    <div className="shadow-2xl rounded-sm bg-white relative transition-transform duration-300 origin-top" style={{
                                        width: 'fit-content',
                                        height: 'fit-content'
                                    }}>
                                        <Page
