...
export default function NewsEventsSection({ onNavigate }: { onNavigate: (path: string) => void }) {
  ...
  return (
    <motion.section
      ref={ref}
      className='py-24 max-w-[1440px] mx-auto px-4 relative'
      initial={{ opacity: 0 }}
    ...
    >
      ...
      <div className='relative z-10'>
        ...
        <div className='grid grid-cols-1 lg:grid-cols-3 gap-6'>
          ...
          >
            <div className='absolute -left-px top-0 w-[3px] h-10 bg-[#e5a812]'></div>
                ...
                <div className='absolute left-3 bottom-3 right-3 text-white pointer-events-none'>
                  <div className='flex items-center gap-2'>
                    <div className='flex flex-col text-center leading-none shrink-0'>
                      <span className='text-3xl font-bold leading-none drop-shadow-md mb-2'>
                        {newsItems[0].date.split('/')[0]}
                      </span>
                      <span className='text-xs font-medium leading-none drop-shadow-md'>
                        {newsItems[0].date.split('/')[1]}-{newsItems[0].date.split('/')[2]}
                      </span>
                    </div>
                    <div className='w-0.5 h-12 bg-white opacity-80' />
                    <h4 className='text-sm font-bold leading-tight line-clamp-2 drop-shadow-md'>
                      {newsItems[0].title}
                    </h4>
                  </div>
                </div>
          ...
          >
              ...
              <div className='relative overflow-hidden'>
                ...
                <div className='absolute left-3 bottom-3 right-3 text-white pointer-events-none'>
                  <div className='flex items-center gap-2'>
                    <div className='flex flex-col text-center leading-none shrink-0'>
                      <span className='text-3xl font-bold leading-none drop-shadow-md mb-2'>
                        {(eventItems[0].date || '').split('/')[0]}
                      </span>
                      <span className='text-xs font-medium leading-none drop-shadow-md'>
                        {(eventItems[0].date || '').split('/')[1]}-{(eventItems[0].date || '').split('/')[2]}
                      </span>
                    </div>
                    <div className='w-0.5 h-12 bg-white opacity-80' />
                    <h4 className='text-sm font-bold leading-tight line-clamp-2 drop-shadow-md'>
            ...
            <div className='grid grid-cols-2 gap-4 mb-6'>
              {eventItems.slice(1).map((item, index) => (
                <div key={index} className='cursor-pointer group' onClick={() => onNavigate(item.link)}>
          ...
          >
            ...
            {/* Large Image */}
            <div className='cursor-pointer group grow' onClick={() => onNavigate(galleryItem.link)}>
